...
def parse_args():
    ...
    parser.add_argument('--name', default='jiu0Monkey_NestedUnet_woDS',
                        help='model name')
...
def main():
    ...
    args = joblib.load('models_nestednet/%s/args.pkl' %val_args.name)
    ...
    joblib.dump(args, 'models_nestednet_2/%s/args.pkl' %args.name)
    ...
    model.load_state_dict(torch.load('models_nestednet_2/%s/model.pth' %args.name))
    model.eval()
